...
exten => h,n,System(/usr/src/sendEmail-v1.56/sendEmail.pl -f from-email@gmail.com -t \${EMAIL} -u \"fax result\" -m \"\${FAXSTATUS} --\${FAXERROR} ---\${REMOTESTATIONID} \" -s smtp.gmail.com -o tls=yes -xu from-email@gmail.com -xp password -o message-charset=UTF-8)
...
exten => h,n,System(/usr/src/sendEmail-v1.56/sendEmail.pl -f from-email@gmail.com -t \${FAX_RX_EMAIL} -u \"Входящий fax  с номера \${STRREPLACE(CALLERID(all),',\\\\')}\" -m \" С номера \${STRREPLACE(CALLERID(all),',\\\\')} на номер \${FROM_DID}\"  -a \${ASTSPOOLDIR}/fax/\${UNIQUEID}.tif -s smtp.gmail.com -o tls=yes -xu from-email@gmail.com -xp password -o message-charset=UTF-8)
